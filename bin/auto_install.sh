# install latest python from source
# (including _sqlite3, readline, bz2 modules), 
sudo apt-get install libsqlite3-dev libreadline-dev libbz2-dev zlib1g-dev openssl libssl-dev
# use latest python to install setuptools and pip (search it)
# use pip to install ipython

#sudo pip install vim_bridge
#sudo pip install virtualenvwrapper

#sudo apt-get install ctags
#sudo apt-get install vim

#sudo apt-get install easystroke

# for install git
#sudo apt-get install libcurl4-openssl-dev libexpat-dev gettext

# for openschool
#sudo apt-get install apache2
#sudo apt-get install php5
#sudo apt-get install libapache2-mod-php5
#sudo apt-get install php5-mysql
#sudo /etc/init.d/apache2 restart

### for centos
sudo yum install zsh
